...
(function() {
  ...
  // talk to the background script and persist to storage
  function notifyGlobal(message, value) {
    if (message !== 'tabSettings' && message !== 'noCI' && message !== 'RGB' && message !== 'blur' && message !== 'dull') {
      var update = {};
      update[message] = value;
      browser.storage.local.set(update);
    }
    browser.runtime.sendMessage({ name: message, message: value }).catch(function() {});
  }
  ...
  function newElement(nodeType, classes, domParent) {
    var
    doc = (document.documentElement && document.documentElement.namespaceURI === 'http://www.w3.org/2000/svg') ? document.implementation.createHTMLDocument(document.URL) : document,
  ...
  function CI(image, kind) {
    ...
    function HelpWindow() {
      ...
      this.popup = function(showIt) {
        if (win === undefined) {
          helpWindow.populate(strings.instructions);
        } else {
    ...
    function drag(e) {
      ...
      if (e.button === 0) {
        switch (e.type) {
          case 'mousedown':
          if (e.target.tagName !== 'IMG' && e.target.id !== 'proxy') {
    ...
    function handleContextMenu(e) {
      // context menus handled via manifest in WebExtensions
    }
    ...
    function respond(message) {
      var m = message.message, n = message.name;
      switch (n) {
        case 'initialize' :
        setStyleClass(m.efct);
        setZoom(m.zoom);
        setEnhancement(m.equi);
        setBackground(isNaN(parseInt(m.bcol, 10)) ? 128 : parseInt(m.bcol, 10), m.auto);
        break;
    ...
    function initCI(image) {
      ...
      browser.runtime.onMessage.addListener(function(msg) { respond(msg); });

      var port = browser.runtime.connect({ name: 'ci-content' });
      port.onMessage.addListener(function(msg) { respond(msg); });
      ...
      // initialization is done, now load settings directly from storage
      browser.storage.local.get(['Zoom','AutoBgColor','BgColor','Equalize','Effect'], function(s) {
        respond({
          name: 'initialize',
          message: {
            zoom: s.Zoom  || false,
            auto: s.AutoBgColor || false,
            bcol: s.BgColor !== undefined ? s.BgColor : 128,
            equi: s.Equalize || false,
            efct: s.Effect || 'none'
          }
        });
      });
  ...
  (function() {
    ...
    var i, singleImage = (function() {
      ...
      // window.SVGDocument is removed in modern browsers — check via namespace instead
      if (document.documentElement && document.documentElement.namespaceURI === 'http://www.w3.org/2000/svg') {
