...
function setupUi () {
  ...
  function handleClick (e) {
    if (!e.target.disabled) {
      switch (e.target) {
        case ui.zoom : settings.Zoom = !settings.Zoom; break; 
        case ui.equi : settings.Equalize = !settings.Equalize; break;
        case ui.inst :
  ...
  function handleMessage (message) {
    ...
    if ( message.target === safari.application.activeBrowserWindow.activeTab ) {
      switch (message.name) {
        ...
        if ( settings.ShowBars ) { bars[0].show(); }
        break;
        case 'svg'       : ui.equi.disabled = ui.inst.disabled = message ? 'disabled' : null; break;
        case 'bgColor'   : ui.bgsl.value = m; break;
        case 'inst'      : instWinVisible = m; ui.inst.className = instWinVisible ? 'pressed' : null; break;
        case 'GoodImage' : ui.equi.disabled = m ? 'disabled' : null; break;
        case 'noImage' : 
        ui.zoom.disabled =
        ui.equi.disabled =
        ui.slbl.className =
